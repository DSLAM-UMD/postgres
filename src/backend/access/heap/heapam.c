		 ...
		 lineoff++, lpp++)
	{
		if (ItemIdIsNormal(lpp))
		{
			...
			if (all_visible)
				...
				valid = HeapTupleSatisfiesVisibility(RelationGetRegion(scan->rs_base.rs_rd),
					   ...
					   bool *all_dead, bool first_call)
{
	...
	for (;;)
	{
		...
		if (!skip)
		{
			...
			valid = HeapTupleSatisfiesVisibility(RelationGetRegion(relation), heapTuple, snapshot, buffer);
					...
					ItemPointer tid)
{
	...
	for (;;)
	{
		 ...
		 * candidate.
		 */
		valid = HeapTupleSatisfiesVisibility(RelationGetRegion(relation), &tp, snapshot, buffer);
					...
					CommandId cid, int options)
{
	...
	/* 
	 * Remotexact 
	 * If this is a remote relation, use a special xid for the xmin of the
	 * locally created tuple.
	 */
	if (RelationIsRemote(relation))
		HeapTupleHeaderSetXmin(tup->t_data, TemporaryTransactionId);
	else
...
l1:
	...
	if (crosscheck != InvalidSnapshot && result == TM_Ok)
	{
		...
		if (!HeapTupleSatisfiesVisibility(RelationGetRegion(relation), &tp, crosscheck, buffer))
			result = TM_Updated;
	}
...
l2:
	...
	{
		/* Perform additional check for transaction-snapshot mode RI updates */
		if (!HeapTupleSatisfiesVisibility(RelationGetRegion(relation), &oldtup, crosscheck, buffer))
		{
			result = TM_Updated;
	...
	else if (relation_is_remote)
		xmax_new_tuple = TemporaryTransactionId;
	else
	...
	if (relation_is_remote)
		HeapTupleHeaderSetXmin(newtup->t_data, TemporaryTransactionId);
	else
						  ...
						  uint16 *result_infomask2)
{
	...
	/*
	 * Remotexact
	 * If this is a remote relation, use a special xid for the xmax of the local write.
	 */
	if (is_remote)
		add_to_xmax = TemporaryTransactionId;	
