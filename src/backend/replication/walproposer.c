...
SendProposerElected(WalKeeper *wk)
{
	...
	{
		/*
		 * End of term is the start of the next except it is the last one; there
		 * it is flush_lsn in case of safekeeper or just the start in case
		 * of proposer (who hasn't wrote anything yet).
		 */
		XLogRecPtr propEndLsn = (i + 1 < propTermHistory.n_entries ?
		                        	propTermHistory.entries[i + 1].lsn :
									propTermHistory.entries[i].lsn);
		XLogRecPtr skEndLsn =
			(i + 1 < th->n_entries ? th->entries[i + 1].lsn : wk->voteResponse.flushLsn);
	...
	lastCommonTerm = i >= 0 ? propTermHistory.entries[i].term : -1;
	elog(LOG,
